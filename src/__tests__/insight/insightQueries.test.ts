...
import { afterEachTest, beforeEachTest, paramsDef, paramsValue } from './common';
...
describe('insightQueries', () => {
  ...
  test('insightsIntegrations', async () => {
    ...
    const response = await graphqlRequest(qry, 'insightsIntegrations', args);

    expect(response[1].value).toEqual(2); // form
    expect(response[5].value).toEqual(8); // gmail
  });
