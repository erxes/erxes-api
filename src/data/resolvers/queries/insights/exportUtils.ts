...
import { Brands, ConversationMessages, Conversations, Integrations, Tags } from '../../../../db/models';
import { IUserDocument } from '../../../../db/models/definitions/users';
import { TAG_TYPES } from '../../../constants';
import { getDateFieldAsStr, getDurationField } from '../aggregationUtils';
import { IAddCellArgs, IListArgs, IListArgsWithUserId, IVolumeReportExportArgs } from './types';
import {
  fixDates,
...
export const generateVolumeReport = async (args: IListArgs, user: IUserDocument) => {
  ...
  const mainResolvedSelector = await getConversationSelector(filterSelector, resolvedSelector, 'closedAt');
  ...
  const messageSelector = await generateMessageSelector({ type: 'volume' });
