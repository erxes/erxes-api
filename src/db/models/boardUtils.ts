...
export const getNewOrder = async ({ collection, stageId, aboveItemId }: ISetOrderParam) => {
  ...
  if (aboveItemId) {
    ...
    const nextItems = await collection
      .find({ stageId, order: { $gt: prevOrder } })
      .sort({ order: 1 })
      .limit(1);
    ...
    const firstItems = await collection
      .find({ stageId })
      .sort({ order: 1 })
      .limit(1);
  ...
  if (!prevOrder) {
    ...
    return round(afterOrder - 0.1 ** (afterLen - 2 - afterDotLen + diffIs1Len), afterLen + diffIs1Len);
  }
  ...
  return round(afterOrder - 0.1 ** (diffLen - 2 - dotLen + is1Len), diffLen + is1Len);
};
